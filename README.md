# Ant Design 扩展组件库

[![NPM version](https://img.shields.io/npm/v/@trionesdev/antd-react-ext.svg?style=flat)](https://npmjs.org/package/@trionesdev/antd-react-ext)
[![NPM downloads](http://img.shields.io/npm/dm/@trionesdev/antd-react-ext.svg?style=flat)](https://npmjs.org/package/@trionesdev/antd-react-ext)

> 基于And Design 组件库的扩展组件库。需要依赖 [ant-design](https://github.com/ant-design/ant-design)

* antd >= 6.5.0

[文档网站](https://trionesdev.github.io/antd-react-ext/components/table-toolbar)

---
## 组件列表
- [x] AppToolbar 应用工具栏
- [x] AvatarEditor 头像编辑器
- [x] DrawerForm 抽屉表单
- [x] EditableDesc 可编辑描述
- [x] ExtCheckbox 扩展Checkbox
- [x] ExtDatePicker 扩展DatePicker
- [x] ExtFormField 扩展FormField
- [x] ExtInput 扩展Input
- [x] ExtInputNumber 扩展InputNumber
- [x] ExtRadio 扩展Radio
- [x] ExtSelect 扩展Select
- [x] ExtSwitch 扩展Switch
- [x] FetchSelect 远程数据的Select
- [x] FetchTreeSelect 远程数据的TreeSelect
- [x] FieldsMapping 字段映射
- [x] FileWrapper 字段包装器
- [x] FormCell 表单项
- [x] GridTable 表格
- [x] ImageUpload 图片上传
- [x] Layout 布局
- [x] ModalForm 模态框表单
- [x] PageHeader 页头
- [x] SearchToolbar 搜索工具栏
- [x] TableToolbar 表格工具栏

## LICENSE

MIT


---

## 关注我们，一起交流
> 留言回复不及时，可以通过关注公众号联系我们
<div style="width: 100%;text-align: center;">
<img src="images/shuque_wx.jpg" width="200px" alt="">
</div>


